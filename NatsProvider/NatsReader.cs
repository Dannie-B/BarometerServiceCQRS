  ...
  where T : class
{
  ...
  private readonly INatsJSStream jetStream;
  private readonly NatsJSContext jetStreamContext;
  ...
  public override async IAsyncEnumerable<T> ReadAllAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
  {
    ...
    await foreach (NatsJSMsg<T> jsMsg in consumer!.ConsumeAsync(serializer: new CommandEventSerializer<T>(), cancellationToken: cts.Token))
