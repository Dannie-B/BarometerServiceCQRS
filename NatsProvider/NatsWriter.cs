  ...
  where T : class
{
  ...
  public override async ValueTask WriteAsync(T item, CancellationToken cancellationToken = default)
  {
    ...
    PubAckResponse ack = await jetStreamContext!.PublishAsync(serializer: new CommandEventSerializer<T>(), subject: config.Subject, data: item, cancellationToken: cts.Token);
    ack.EnsureSuccess();
  }
}
